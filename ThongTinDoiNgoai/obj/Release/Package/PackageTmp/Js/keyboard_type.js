...
function banphimso(textId, keyboardId, imgIcon,actionId) {
...
var keyboard = new Keyboard({
    onChange: input => onChangeNew(input, textId),
        onKeyPress: button => onKeyPress(button,keyboard,keyboardId,actionId),
    layout: {
        default: ["1 2 3", "4 5 6", "7 8 9", "{shift} 0 _", "{bksp}"],
    shift: ["! / #", "$ % ^", "& * (", "{shift} ) +", "{bksp}"]
},
...
function banphimchu(textId, keyboardId,  imgIcon, actionId) {
...
function onKeyPress(button,keyboard,keyboardId,actionId) {
    ...
    */
    if(button === "{enter}") {
        ...
        var x = document.getElementById(actionId);
        console.log(x);
        x.click();
    }
}
